...
for t in range(1):
    ...
    for img_batch, labels_batch in train_loader:
        ...
        # Positive Phase
        v0, prob_h0 = qa_sampler(img_batch.flatten(1),num_reads=POS_READS)
        # Negative Phase
        vk, prob_hk = qa_sampler(num_reads=NEG_READS,num_spin_reversal_transforms=4)
...
# def best_effort(model, sampler, seed=None):
model = bm
sampler = qa_sampler
""" Best-effort embedding method """

T = sampler.device.to_networkx_graph()
len(T.nodes)

cache = minorminer.busclique.busgraph_cache(T)
biclique = cache.largest_balanced_biclique()
clique = cache.largest_clique()

fig = plt.figure(figsize=(16,16))
dnx.draw_pegasus_embedding(T,biclique,node_size=15)
len(biclique)

fig = plt.figure(figsize=(16,16))
dnx.draw_pegasus_embedding(T,clique,node_size=15)
len(clique)

M = sampler.device.properties['topology']['shape']
T100 = dnx.pegasus_graph(*M)
cache100 = minorminer.busclique.busgraph_cache(T100)
biclique100 = cache100.largest_balanced_biclique()
clique100 = cache100.largest_clique()
len(T100.nodes)

fig = plt.figure(figsize=(16,16))
dnx.draw_pegasus_embedding(T100,biclique100,node_size=15)
len(biclique100)


fig = plt.figure(figsize=(16,16))
dnx.draw_pegasus_embedding(T100,clique100,node_size=15)
len(clique100)
