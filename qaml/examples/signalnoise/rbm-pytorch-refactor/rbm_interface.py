...
class RBMInterface:
		...
		for epoch in self.progress_bar:
			...
			if self.use_validation:
				...
				for i, (data, target) in enumerate(self.validation_set):
					...
					validation_free_energy[i] = self.rbm.free_energy(v).data.item()


				for i, (data, target) in enumerate(self.comparison_set):
					data = data.type(self.dtype)
					v = Variable(data.view(-1, self.args.visible_n))
					comparison_free_energy[i] = self.rbm.free_energy(v).data.item()
	...
	def train_step(self, data):
		...
		return loss.data.item(), reconstruction_error.data.item(), data_free_energy.data.item()
